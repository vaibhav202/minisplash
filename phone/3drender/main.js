...
const miniSplash = 'https://api.unsplash.com/search/photos/?per_page=30&order_by=latest&content_filter=high&orientation=portrait&query=3d%20render&client_id=jNPKpYi80xqW7F7Ot37Mqsaft-NsbggphFw0WRAyw-E';
fetch(miniSplash)
.then(response =>{
    if(response.ok){
...
const miniSplash2 = 'https://api.unsplash.com/search/photos/?page=2&per_page=30&order_by=latest&content_filter=high&orientation=portrait&query=3d%20render&client_id=jNPKpYi80xqW7F7Ot37Mqsaft-NsbggphFw0WRAyw-E';
fetch(miniSplash2)
.then(response =>{
    if(response.ok){
        return response.json();
